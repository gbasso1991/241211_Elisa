...
ciclos_100 = glob(os.path.join('NF241126_Citrato_100kHz','**', '*ciclo_promedio*'),recursive=True)
ciclos_100.sort()
labels_100 = [os.path.split(s)[-1].split('bobN5')[-1].split('_')[1] for s in ciclos_100]
...
_,_,_,H100_5,M_100_5,meta_100_5=lector_ciclos(ciclos_100[4]) 
_,_,_,H100_6,M_100_6,meta_100_6=lector_ciclos(ciclos_100[5]) 
...
# Ciclos 100
ax.plot(H100_1, M_100_1, label=labels_100[0] + f' = {max(H100_1)/1000:.1f} kA/m')
ax.plot(H100_2, M_100_2, label=labels_100[1] + f' = {max(H100_2)/1000:.1f} kA/m')
ax.plot(H100_3, M_100_3, label=labels_100[2] + f' = {max(H100_3)/1000:.1f} kA/m')
ax.plot(H100_4, M_100_4, label=labels_100[3] + f' = {max(H100_4)/1000:.1f} kA/m')
ax.plot(H100_5, M_100_5, label=labels_100[4] + f' = {max(H100_5)/1000:.1f} kA/m')
ax.plot(H100_6, M_100_6, label=labels_100[5] + f' = {max(H100_6)/1000:.1f} kA/m')


# ax.set_title('100', fontsize=14

ax.set_ylabel('M (A/m)')
ax.grid()
ax.legend(ncol=1)
ax.set_xlabel('H (A/m)')
ax.set_title('NF241126 @Citrato - 100kHz')
plt.savefig('NF241126_Citrato_100kHz', dpi=100, facecolor='w')
plt.show()
#%% Levanto archivos resultados
res_100 = glob(os.path.join('NF241126_Citrato_100kHz','**','*resultados*'),recursive=True)
...
#%% SAR y Tau
(SAR_100,SAR_100_err,tau_100,Hc_100, H_max_100)=([],[],[],[],[])
...
for path in res_100:
    ...
    SAR_100.append(np.mean(SAR))
    SAR_100_err.append(np.std(SAR))
    tau_100.append(np.mean(tau))
    H_max_100.append(np.mean(campo_max)/1000)
...
ax.errorbar(x=H_max_100,y=SAR_100,yerr=SAR_100_err,capsize=5,fmt='.-')

ax.grid()
ax.set_xlabel('H$_{max}$ (kA/m)')
ax.set_ylabel('SAR (W/g)')
plt.savefig('SAR_vs_Hmax.png',dpi=300)
